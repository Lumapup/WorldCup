...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' AND (team_id = winner_id OR team_id = opponent_id) ORDER BY name;")"
